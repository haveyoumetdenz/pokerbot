    ...
    def self_play_cfr_traverse(self, state, iteration, opponent_agents, depth=0):
        ...
        if current_player == self.player_id:
            ...
            for i, a in enumerate(legal_action_ids):
                if i < len(advantages_np):
                    val = advantages_np[i]
                    try:
                        val = float(val.item()) if hasattr(val, "item") and np.size(val) == 1 else float(np.array(val).flatten()[0])
                    except Exception:
                        val = float(val)
                    advantages_masked[a] = max(val, 0)
                else:
                    # SHAPE MISMATCH: NOT ENOUGH VALUES, assign 0
                    advantages_masked[a] = 0
            ...
            for action_id in legal_action_ids:
                try:
                    pokers_action = self.action_type_to_pokers_action(action_id, state)
            ...
            for action_id in legal_action_ids:
                ...
                self.advantage_memory.add((
                    ...
                    np.zeros(20),  # Placeholder for opponent_features (adjust dim as needed)
                    action_id,
                    0.0,  # bet_size placeholder
    ...
    def mixed_checkpoints_cfr_traverse(self, state, iteration, opponent_agents, depth=0):
        ...
        if current_player == self.player_id:
            ...
            self.strategy_memory.append((
                ...
                np.zeros(20),  # Placeholder for opponent_features
                strategy_full,
                0.0,  # bet_size placeholder
                iteration
            ))
