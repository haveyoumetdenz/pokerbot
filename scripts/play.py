...
def detect_model_type(models_dir):
    """Detect if models_dir contains interpretable trees (.pkl) or DeepCFR models (.pt)."""
    ...
    # Check for tree files (interpretable)
    tree_files = [f for f in pkl_files if any(tree in os.path.basename(f) for tree in ['preflop', 'flop', 'turn', 'river', 'tree'])]
...
def select_random_models(models_dir, num_models=5, model_pattern="*.pt"):
    ...
    """
    # Check if this is an interpretable agent directory (contains .pkl tree files)
    if model_pattern == "*.pt":
        pkl_files = glob.glob(os.path.join(models_dir, "*.pkl"))
        tree_files = [f for f in pkl_files if any(tree in os.path.basename(f) for tree in ['preflop', 'flop', 'turn', 'river', 'tree'])]
        if tree_files:
            return []  # Interpretable doesn't need individual model files
    ...
    while True:
        ...
        for i in range(6):
            if i == player_position:
                ...
                # AI opponents: Use StrategicAgent, DeepCFR, or Random based on opponent_type
                if opponent_type == 'strategic':
                    # Use StrategicAgent with specified tightness
                    tightness_levels = ['very_tight', 'tight', 'average', 'loose', 'very_loose', 'any_two']
                    # Distribute different tightness levels across opponents for variety
                    tightness_idx = (i - 1) % len(tightness_levels)
                    opp_tightness = tightness_levels[tightness_idx] if opponent_tightness == 'mixed' else opponent_tightness
                    agents.append(StrategicAgent(i, tightness=opp_tightness))
                    print(f"Using StrategicAgent for Player {i} (tightness: {opp_tightness})")
                elif opponent_type == 'deepcfr':
                    # Try to use DeepCFR agents
                    model_idx = (i - 1) if i > player_position else i
                    
                    # Try to find DeepCFR models directory
                    deepcfr_dir = None
                    if models_dir:
                        model_type = detect_model_type(models_dir)
                        if model_type == 'deepcfr':
                            deepcfr_dir = models_dir
                    # Also check default 'models' directory for DeepCFR
                    if deepcfr_dir is None and os.path.isdir('models'):
                        if detect_model_type('models') == 'deepcfr':
                            deepcfr_dir = 'models'
                    
                    # Try to load DeepCFR agent for opponent
                    if deepcfr_dir:
                        deepcfr_paths = select_random_models(deepcfr_dir, num_models, "*.pt")
                        if deepcfr_paths and model_idx < len(deepcfr_paths):
                            try:
                                agent = DeepCFRAgent(player_id=i, num_players=6, device=device)
                                agent.load_model(deepcfr_paths[model_idx])
                                agents.append(agent)
                                print(f"Loaded DeepCFR model for Player {i}: {os.path.basename(deepcfr_paths[model_idx])}")
                            except Exception as e:
                                print(f"Error loading DeepCFR model for Player {i}: {e}")
                                print("Using random agent instead")
                                agents.append(RandomAgent(i))
                        else:
                            agents.append(RandomAgent(i))
                            print(f"No DeepCFR models available, using random agent for Player {i}")
                    else:
                        agents.append(RandomAgent(i))
                        print(f"No DeepCFR models found, using random agent for Player {i}")
                else:
        ...
        while not state.final_state:
            ...
            if current_player == player_position:
                ...
                # Show bot suggestion: Try interpretable first, then DeepCFR
                # Check for interpretable models (default location or from models_dir)
                interpretable_dir = None
                if models_dir and detect_model_type(models_dir) == 'interpretable':
                    interpretable_dir = models_dir
                else:
                    # Check default interpretable output location
                    default_interp_dir = 'interpretable_output/models/models'
                    if os.path.isdir(default_interp_dir) and detect_model_type(default_interp_dir) == 'interpretable':
                        interpretable_dir = default_interp_dir
                
                if interpretable_dir:
                    # Use interpretable agent for suggestions
                    try:
                        interpretable_bot = InterpretablePokerAgent(player_id=player_position, tree_dir=interpretable_dir)
                        explanation = interpretable_bot.explain_decision(state)
                        
                        print("\n🤖 Interpretable Bot Suggestion:")
                        print(f"  Action: {explanation['action'].upper()}")
                        print(f"  Confidence: {explanation['confidence']:.1%}")
                        
                        # Show recommended bet size if action is raise
                        if explanation.get('recommended_bet_size') is not None:
                            bet_size = explanation['recommended_bet_size']
                        ...
                        print(f"\n⚠️  Error loading interpretable agent: {e}")
                        # Fall through to DeepCFR suggestions
                        interpretable_dir = None
                
                if not interpretable_dir:
                    # Use DeepCFR agent for suggestions (if available)
                    if models_dir and detect_model_type(models_dir) == 'deepcfr':
                        ai_bot = DeepCFRAgent(player_id=player_position, num_players=6, device=device)
                        ai_bot_ckpt = None
                        ckpts = sorted(glob.glob(os.path.join(models_dir, "*.pt")))
                        if ckpts:
                            ai_bot_ckpt = ckpts[-1]
                            ai_bot.load_model(ai_bot_ckpt)
                        if ai_bot_ckpt:
                            import torch.nn.functional as F
                            from src.core.model import encode_state
                            state_tensor = torch.FloatTensor(
                                encode_state(state, player_position)
                            ).unsqueeze(0).to(device)
                            with torch.no_grad():
                                logits, bet_size_pred = ai_bot.strategy_net(state_tensor)
                                probs = F.softmax(logits, dim=1)[0].cpu().numpy()
                            legal_action_types = ai_bot.get_legal_action_types(state)
                            action_names = {0: "Fold", 1: "Check/Call", 2: "Raise"}
                            print("\n🤖 Bot Action Probabilities (for your seat):")
                            for a in legal_action_types:
                                print(f"  {action_names[a]}: {probs[a]*100:.1f}%")
                            suggested_action = ai_bot.choose_action(state)
                            print(f"🤖 Bot Suggestion: {get_action_description(suggested_action)}")
                        else:
                            print("\n🤖 Bot Suggestion: (No DeepCFR model loaded)")
                    else:
                        # Try to find DeepCFR in default 'models' directory
                        if os.path.isdir('models') and detect_model_type('models') == 'deepcfr':
                            ai_bot = DeepCFRAgent(player_id=player_position, num_players=6, device=device)
                            ckpts = sorted(glob.glob(os.path.join('models', "*.pt")))
                            if ckpts:
                                ai_bot.load_model(ckpts[-1])
                                import torch.nn.functional as F
                                from src.core.model import encode_state
                                state_tensor = torch.FloatTensor(
                                    encode_state(state, player_position)
                                ...
                                print("\n🤖 Bot Suggestion: (No model loaded)")
                        else:
                            print("\n🤖 Bot Suggestion: (No model loaded)")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--opponent-type', type=str, default='strategic',
                       choices=['random', 'strategic', 'deepcfr'],
                       help='Type of opponent: random, strategic (with calling ranges), or deepcfr (default: strategic)')
