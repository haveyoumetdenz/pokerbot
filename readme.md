# DeepCFR Poker AI with Interpretable Decision Trees

## 🎯 Overview

This project implements a **hybrid poker AI** that combines Deep Counterfactual Regret Minimization (Deep CFR) with interpretable decision trees. The system achieves complete transparency in poker decision-making while maintaining competitive performance through knowledge distillation.

### Key Features
- **🧠 Deep CFR Training**: Neural networks learn optimal poker strategies
- **🌳 Interpretable Decision Trees**: CART trees provide human-readable explanations
- **📊 Complete Transparency**: Every decision explained with reasoning
- **🎮 Interactive Play**: Play against the AI with full decision explanations
- **📈 Performance Analysis**: Detailed evaluation and comparison tools
...
### Training the Interpretable AI

#### 1. Train Deep CFR with Data Collection
```bash
# Set Python path (required for all commands)
export PYTHONPATH=/path/to/deepcfr-texas-no-limit-holdem-6-players-main

# Train Deep CFR and collect interpretable data
python -m src.training.train_interpretable --iterations 1000 --traversals 200
```

#### 2. Train Decision Trees
```bash
# Train CART decision trees on collected data
python -m src.interpretable.tree_trainer --data-dir interpretable_output/data --output-dir interpretable_output/models
```

#### 3. Evaluate Performance
```bash
# Evaluate interpretable agent against random opponents
python scripts/evaluate_interpretable.py --tree-dir interpretable_output/models --num-games 100
```

### Playing Against the AI

#### Play Against Interpretable Agent
```bash
# Play against the interpretable decision tree agent
python scripts/play.py --models-dir interpretable_output/models --position 0 --stake 200
```

#### Play Against Deep CFR Neural Networks
```bash
# Play against the original Deep CFR neural networks (harder)
python scripts/play.py --models-dir models --position 0 --stake 200
```

#### Test Decision Explanations
```bash
# See decision explanations for different scenarios
python test_interpretable_decisions.py
```
...
# Training with interpretable data collection
python -m src.training.train_interpretable --iterations 1000 --traversals 200

# Advanced training with opponent modeling
python -m src.training.train --iterations 2000 --traversals 500 --mixed
```

### Decision Tree Training
```bash
# Train decision trees from collected data
python -m src.interpretable.tree_trainer --data-dir interpretable_output/data --output-dir interpretable_output/models

# Train with custom parameters
python -m src.interpretable.tree_trainer --data-dir interpretable_output/data --output-dir interpretable_output/models --max-depth 15
```
...
### Interactive Play
```bash
# Play against interpretable agent (easier, with explanations)
python scripts/play.py --models-dir interpretable_output/models --position 0

# Play against Deep CFR neural networks (harder, no explanations)
python scripts/play.py --models-dir models --position 0

# Play with custom settings
python scripts/play.py --models-dir models --position 2 --stake 500 --sb 2 --bb 4
```

### Evaluation and Analysis
```bash
# Evaluate interpretable agent performance
python scripts/evaluate_interpretable.py --tree-dir interpretable_output/models --num-games 1000

# Compare interpretable vs neural network
python scripts/evaluate_interpretable.py --tree-dir interpretable_output/models --neural-model models/checkpoint_iter_100.pt --num-games 100

# Visualize decision trees
python scripts/visualize_interpretable_trees.py --tree-dir interpretable_output/models
```
...
python scripts/poker_gui.py
```

## 📈 Performance Results

### Interpretable Agent Performance
- **vs Random Opponents**: 100% win rate, ~200 BB/100 profit
- **Decision Accuracy**: 20-37% vs CFR ground truth
- **Decision Time**: <1ms per decision
- **Interpretability**: Complete transparency with human-readable explanations

### Deep CFR Neural Network Performance
- **vs Random Opponents**: 100% win rate, higher profit margins
- **vs Human Players**: Competitive performance
- **Decision Time**: ~5-10ms per decision
- **Interpretability**: Black-box decisions (no explanations)

## 🎯 Key Commands Summary

| Purpose | Command |
|---------|---------|
| **Train Deep CFR** | `python -m src.training.train_interpretable --iterations 1000` |
| **Train Decision Trees** | `python -m src.interpretable.tree_trainer --data-dir interpretable_output/data` |
| **Play vs Interpretable** | `python scripts/play.py --models-dir interpretable_output/models` |
| **Play vs Deep CFR** | `python scripts/play.py --models-dir models` |
| **Evaluate Performance** | `python scripts/evaluate_interpretable.py --tree-dir interpretable_output/models` |
| **Test Explanations** | `python test_interpretable_decisions.py` |
| **Visualize Trees** | `python scripts/visualize_interpretable_trees.py --tree-dir interpretable_output/models` |
...
export PYTHONPATH=/path/to/deepcfr-texas-no-limit-holdem-6-players-main
python scripts/play.py --models-dir models
```
...
```bash
# Train models first
python -m src.training.train_interpretable --iterations 1000
python -m src.interpretable.tree_trainer --data-dir interpretable_output/data
```

## 📚 Documentation

- **[System Architecture Visualization](./SYSTEM_ARCHITECTURE_VISUALIZATION.md)**: Complete visual guide to the system
- **[Interpretable README](./INTERPRETABLE_README.md)**: Detailed interpretable AI documentation
- **[Learning Guide](./LEARNING_GUIDE.md)**: How to learn poker from the AI
- **[Future Improvements](./FUTURE_IMPROVEMENTS.md)**: Planned features and enhancements
...
1. **Release Schedule**: I'll be publishing regular updates with new features and improvements.
   - v2.1 (April 2025): ✅ Fixed bet calculation for stable training, ✅ Implemented opponent modeling capabilities, ✅ Interpretable decision trees, Expanded bet sizing options and performance optimizations, Advanced architectures and training methods
